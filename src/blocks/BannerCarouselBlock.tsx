...
export default function BannerCarouselBlock({
  ...
  /** Multiple resolved banners = carousel + dots; single = static, no dots (ignore stale config). */
  const useCarousel = banners.length > 1;
  if (banners.length === 0) {
    return <EmptySectionState title={title || 'Offers'} />;
  }
  if (!useCarousel) {
    return (
      <BannerSingleTap
  ...
  return <BannerSection banners={banners} blockStyle={blockStyle} isFirstBannerBlock={isFirstBannerBlock} />;
}
