...
interface DealsSectionProps {
  ...
  /** First N horizontal cards use high image priority (above-the-fold carousel). */
...
export default function DealsSection({
  ...
  /** Modal performs `addToCart`; sync selected line for cards. */
  ...
  return (
    <View style={styles.container}>
      ...
      {!loading && products.length === 0 && (
        <Text style={styles.emptyHint}>No products to show here right now.</Text>
      )}
      <ScrollView
        horizontal
        ...
        {products.map((product, index) => (
          ...
          >
            <ProductCard
              ...
              imagePriority={
                index < highPriorityImageCount ? 'high' : highPriorityImageCount > 0 ? 'low' : 'normal'
              }
            />
          </View>
        ))}
...
const styles = StyleSheet.create({
  container: {
    ...
    paddingVertical: Theme.spacing.sectionCardGap,
    gap: 12,
